...
void pni_session_update_incoming_lwm(pn_session_t *ssn) {
  if (ssn->incoming_capacity) {
    ...
    // New API.
    // Only need to deal with default.  May be called several times, last is sending BEGIN frame.
...
int pn_session_set_incoming_window_and_lwm(pn_session_t *ssn, pn_frame_count_t window, pn_frame_count_t lwm) {
  ...
  if (!window || (lwm && lwm > window))
    return PN_ARG_ERR;
  // Vales fixed after open for simplicity.  AMPQ actually allows dynamic change with risk
  ...
  ssn->incoming_window_lwm = lwm;  // Runtime value will be updated at session BEGIN frame.
  return 0;
}
