...
reconnect_timer::reconnect_timer(uint32_t min, int32_t max, uint32_t increment,
                                 bool doubling, int32_t max_retries, int32_t timeout) :
    min_delay_(min), max_delay_(max), increment_(increment), doubling_(doubling),
...
int reconnect_timer::next_delay() {
    ...
    pn_timestamp_t now = pn_reactor_now(pn_cast(reactor_));

    if (retries_ == 1) {
        if (timeout_ >= 0)
            timeout_deadline_ = now + timeout_;
        next_delay_ = min_delay_;
    } else if (retries_ == 2) {
