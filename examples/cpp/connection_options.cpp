...
using proton::connection_handler;
using proton::max_frame_size;
using proton::idle_timeout;
  ...
  public:
    ...
    void on_start(proton::event &e) {
        e.container().connect(url,
            connection_options(connection_handler(&conn_handler), max_frame_size(2468)));
    }
...
int main(int argc, char **argv) {
    try {
        ...
        container.client_connection_options(
            connection_options(max_frame_size(12345), idle_timeout(15000)));
        container.run();
        return 0;
